...
const ProfileSettings = ({handleUpdate}) => {
  ...
  const handlePhoneChange = (value) => {
    //caso especial con react-phone el valor que captura el input necesita tener su propio handler
  ...
  const handleSubmit = async (event) => {
    //los campos no deben ser limpiados
    ...
    try {
      const { data } = await axios.put(
        `https://tu-suenio-back.onrender.com/user/${dataUser.id}/modify`,
        formData
      );
      if (!data.error) {
        setDataUser(data);
    ...
    <div className={styles.contenedorPrincipal}>
      <div className={styles.contenedorSecundario}>
        <div className={styles.contenedorPerfil}>
          <h1 className={styles.nombreApellido}>
            {nombreCompleto.length > 1 ? nombreCompleto : "Nombre Apellido"}
          </h1>
          ...
          <div className={styles.divAvatar}>
            {dataUser.image === "" || dataUser.image === null ? (
              <img
                ...
                className={
                  !isEditing
                    ? `${styles.fotoAvatar} ${styles.disabled}`
                    : styles.fotoAvatar
                }
            ...
            <h1
              className={
                !isEditing
                  ? `${styles.divIconDeleteImage} ${styles.disabled}`
                  : styles.divIconDeleteImage
              }
          ...
          <label
            className={
              !isEditing
                ? `${styles.labelSubirFoto} ${styles.disabled}`
                : styles.labelSubirFoto
            }
            htmlFor="subirFoto"
          >
            Subir Foto
          ...
          <input
            type="file"
            className={styles.inputSeleccionarArchivo}
            id="subirFoto"
            onChange={handleImageChange}
            disabled={!isEditing}
          />
          <div className={styles.divDescripcionSubirFoto}>
            <p className={styles.descripcionSubirFoto}>
              Sube una nueva foto, la imagen se redimensionará automáticamente
            </p>
        ...
        <form
          className={styles.contenedorConfigPerfil}
          onSubmit={(event) => {
            handleSubmit(event);
          }}
        >
          ...
          <div>
            <div className={styles.divNombreConfigPerfil}>
              <h1 className={styles.configuracionPerfil}>
            ...
            <div className={styles.containerInputs}>
              ...
              <div className={styles.inputsDerecha}>
                ...
                <div className={styles.containerButtons}>
                  <button
                    className={
                      !isEditing ? styles.botonEditar : styles.botonCancelar
                    }
                    onClick={() => {
          ...
          >
            ...
            <div className={styles.containerButtonsModal}>
              ...
              <img
                src={Huellas}
                alt="huellas"
                className={styles.huellasBottomModal}
              />
            </div>
          </ReactModal>
        </form>
